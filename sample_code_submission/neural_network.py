...
class NeuralNetwork:
        ...
        if Load_Classifier == True:
            ...
            if not os.path.exists("%s/Saved_Classifier/NN" % (current_dir)):
                ...
                self.model = load_model(
                    "%s/Saved_Classifier/NN/model_NN_trained_wt_%s_events_seed%s.h5"
                    % (current_dir, train_size, seed)
                )
                self.scaler = joblib.load(
                    "%s/Saved_Classifier/NN/scaler_NN_trained_wt_%s_events_seed%s.pkl"
                    % (current_dir, train_size, seed)
                )
    ...
    def predict(self, test_data):
        ...
        return self.model.predict(test_data, batch_size=4_096).flatten().ravel()
