...
def compute_mu(
    ...
    if method == "Direct":  # Based on N=mu*S+b
        ...
        def UNLL_mu (threshold,mu_init,saved_info_hold,weight_test,score_test):
            ...
            def UNLL_mu_computation (mu_init) :
                ...
                plt.savefig(
                    ...
                    % (current_dir,ModelType, round(mu,4),threshold, NbTrain, NbHoldout, NbValidation)
                )
                plt.show()
                plt.close()
        ...
        mu, del_mu_stat,del_mu_stat_inf,del_mu_stat_sup=UNLL_mu(threshold=threshold,mu_init=mu_init,saved_info_hold=saved_info_hold,weight_test=weight_test,score_test=score_test)

    elif method == "BNLL":
        
        def BNLL_mu (theshold,nb_bins,mu_init,weight_exp_hold,label_exp_hold,score_exp_hold,score_test,weight_test) :
            
            def BNLL_mu_parameter (theshold,nb_bins,weight_exp_hold,label_exp_hold,score_exp_hold,weight_test,score_test):
            ...
            n_obs_test_bnll,sig_exp_holdout_bnll,bkg_exp_holdout_bnll=BNLL_mu_parameter(
                theshold=theshold,nb_bins=nb_bins,
        ...
        def BNLL_syst_mu (theshold,nb_bins,mu_init,weight_exp_hold,label_exp_hold,score_exp_hold,score_test,weight_test):
            ...
            def BNLL_syst_mu_fitting_param(nb_bins) :
                ...
                for i in range(3):   #######ATTENTION BESOIN DE METTRE A 3 POUR SOFT MET
                    if os.path.isfile(
                        "current_dir/Fitting_Parameters/one_bkg/%s_%sbins_2orderFittingParam.npz" % (SystName[i], nb_bins)
                    ):
            ...
            def Cost_nll_syst(mu ,tes , jes): # tes, jes, soft_met):
                sig_exp_hold_biaised = ( sig_exp_hold_unbiaised  
                                        ###Below tes
                ...
                bkg_exp_hold_biaised = ( bkg_exp_hold_unbiaised 
                                         ###Below tes
            ...
            def BNLL_syst_mu_computation (mu_init ,tes_init,jes_init): # ,tes_init ,jes_init ,soft_met_init):
                ...
                m_bnll_syst = Minuit(Cost_nll_syst, mu=mu_init, tes=tes_init,jes=jes_init )# tes=tes_init, jes=jes_init, soft_met=soft_met_init)
            ...
            return BNLL_syst_mu_computation(mu_init=mu_init ,tes_init=1 ,jes_init=1 ) #tes_init=1 ,jes_init=1 ,soft_met_init=0)

        mu, del_mu_stat,del_mu_stat_inf,del_mu_stat_sup, tes, jes, soft_met =BNLL_syst_mu(
                                    theshold=threshold,nb_bins=nb_bins,mu_init=mu_init,
                                    weight_exp_hold=weight_exp_hold,label_exp_hold=label_exp_hold,score_exp_hold=score_exp_hold,
                                    weight_test=weight_test,score_test=score_test)
        ...
        def BNLL_syst_norm_mu (theshold,nb_bins,mu_init,weight_exp_hold,label_exp_hold,score_exp_hold,score_test,weight_test):
            
            def BNLL_syst_norm_mu_parameter(threshold,nb_bins,
                                            weight_exp_hold,label_exp_hold,score_exp_hold,weight_test,score_test):
                weight_ROIscore_exp_holdout = weight_exp_hold[score_exp_hold > threshold]
                detailed_labels_Roiscore_exp_holdout = detailed_labels_exp_hold[score_exp_hold > threshold]
                score_ROIscore_exp_holdout = score_exp_hold[score_exp_hold > threshold]
                score_ROIscore_test = score_test[score_test > threshold]
                weight_ROIscore_test = weight_test[score_test > threshold]
                ...
                bkg_scale_ztautau_exp_hold = np.histogram(
                    ...
                    bins=Bins_edges,
                    weights=weight_ROIscore_exp_holdout[detailed_labels_exp_hold == "ztautau"],)[0]

                bkg_ttbar_exp_hold = np.histogram(
                    score_ROIscore_exp_holdout[detailed_labels_exp_hold == "ttbar"],
                    bins=Bins_edges,
                    weights=weight_ROIscore_exp_holdout[detailed_labels_exp_hold == "ttbar"],)[0]

                bkg_diboson_exp_hold = np.histogram(
                    score_ROIscore_exp_holdout[detailed_labels_exp_hold == "diboson"],
                    bins=Bins_edges,
                    weights=weight_ROIscore_exp_holdout[detailed_labels_exp_hold == "diboson"],)[0]
                
                n_obs_test_biaised = np.histogram(
                    score_ROIscore_test, bins=Bins_edges, weights=weight_ROIscore_test)[0]

                return n_obs_test_biaised, sig_exp_hold, bkg_scale_ztautau_exp_hold,bkg_ttbar_exp_hold,bkg_diboson_exp_hold

            n_obs_test_biaised, sig_exp_hold, bkg_scale_ztautau_exp_hold,bkg_ttbar_exp_hold,bkg_diboson_exp_hold=BNLL_syst_norm_mu_parameter(
                        threshold=threshold,nb_bins=nb_bins,
                        weight_exp_hold=weight_exp_hold,label_exp_hold=label_exp_hold,
            ...
            def BNLL_syst_norm_mu_computation (mu_init ,bkg_scale_init, ttbar_scale_init, diboson_scale_init): # bkg_scale_init, ttbar_scale_init, diboson_scale_init ,tes_init ,jes_init ,soft_met_init):

                m_bnll_syst = Minuit(Cost_nll_syst_norm, mu=mu_init, bkg_scale=bkg_scale_init, ttbar_scale=ttbar_scale_init, diboson_scale=diboson_scale_init )# bkg_scale=bkg_scale_init, ttbar_scale=ttbar_scale_init, diboson_scale=diboson_scale_init tes=tes_init, jes=jes_init, soft_met=soft_met_init)
                m_bnll_syst.limits["mu"] = (0, None)
            ...
            return BNLL_syst_norm_mu_computation(mu_init=mu_init ,bkg_scale_init=0, ttbar_scale_init=0, diboson_scale_init=0 ) #tes_init=1 ,jes_init=1 ,soft_met_init=0)

        mu, del_mu_stat,del_mu_stat_inf,del_mu_stat_sup, bkg_scale ,ttbar_scale,  diboson_scale =BNLL_syst_norm_mu(
                                    theshold=threshold,nb_bins=nb_bins,mu_init=mu_init,
                                    weight_exp_hold=weight_exp_hold,label_exp_hold=label_exp_hold,score_exp_hold=score_exp_hold,
                                    weight_test=weight_test,score_test=score_test)
        tes=1
        ...
        def BNLL_all_syst_mu (theshold,nb_bins,mu_init,weight_exp_hold,label_exp_hold,score_exp_hold,score_test,weight_test):
            import os
            from systematic_analysis import Polynomial_Reg_Model_forced_jes_tes,Polynomial_Reg_Model_forced_soft_met
            ...
            def BNLL_all_syst_mu_parameter(threshold,nb_bins,
                                            weight_exp_hold,label_exp_hold,score_exp_hold,weight_test,score_test):
                weight_ROIscore_exp_holdout = weight_exp_hold[score_exp_hold > threshold]
                detailed_labels_Roiscore_exp_holdout = detailed_labels_exp_hold[score_exp_hold > threshold]
                score_ROIscore_exp_holdout = score_exp_hold[score_exp_hold > threshold]
                ...
                return n_obs_test_biaised, sig_exp_hold_unbiaised, bkg_scale_ztautau_exp_hold,bkg_ttbar_exp_hold,bkg_diboson_exp_hold

            n_obs_test_biaised, sig_exp_hold_unbiaised, bkg_scale_ztautau_exp_hold,bkg_ttbar_exp_hold,bkg_diboson_exp_hold=BNLL_all_syst_mu_parameter(
                        threshold=threshold,nb_bins=nb_bins,
                        weight_exp_hold=weight_exp_hold,label_exp_hold=label_exp_hold,
                        score_exp_hold=score_exp_hold,
                        weight_test=weight_test,score_test=score_test)
            ...
            def Cost_nll_all_syst(mu ,bkg_scale, ttbar_scale, diboson_scale,tes, jes, soft_met): # tes, jes, soft_met, bkg_scale, ttbar_scale, diboson_scale :
                ...
                bkg_scale_ztautau_exp_hold_biaised = ( bkg_scale_ztautau_exp_hold
                                                      # Below tes
                            + np.array([Polynomial_Reg_Model_forced_jes_tes(tes, *FitingData[0]["fit_param"][j][1])for j in range(nb_bins) 
                            ])
                                                      # Below jes
                ...
                bkg_ttbar_exp_hold_exp_hold_biaised = ( bkg_ttbar_exp_hold
                                                        # Below tes
                ...
                bkg_diboson_exp_hold_biaised =( bkg_diboson_exp_hold
                                                # Below tes
            ...
            def BNLL_all_syst_mu_computation (mu_init ,tes_init ,jes_init ,soft_met_init, bkg_scale_init, ttbar_scale_init, diboson_scale_init): # bkg_scale_init, ttbar_scale_init, diboson_scale_init ,tes_init ,jes_init ,soft_met_init):
                ...
                m_bnll_syst = Minuit(Cost_nll_all_syst, mu=mu_init,tes=tes_init ,jes=jes_init ,soft_met=soft_met_init, bkg_scale=bkg_scale_init, ttbar_scale=ttbar_scale_init, diboson_scale=diboson_scale_init )# bkg_scale=bkg_scale_init, ttbar_scale=ttbar_scale_init, diboson_scale=diboson_scale_init tes=tes_init, jes=jes_init, soft_met=soft_met_init)
                m_bnll_syst.limits["mu"] = (0, None)
                m_bnll_syst.limits["tes"] = (0.9, 1.1)
                m_bnll_syst.limits["jes"] = (0.9, 1.1)
                m_bnll_syst.limits["soft_met"] = (0, 5)
            ...
            return BNLL_all_syst_mu_computation(mu_init=mu_init ,tes_init=1 ,jes_init=1 ,soft_met_init=0, bkg_scale_init=0, ttbar_scale_init=0, diboson_scale_init=0 ) #tes_init=1 ,jes_init=1 ,soft_met_init=0,bkg_scale_init=0, ttbar_scale_init=0, diboson_scale_init=0)

        mu, del_mu_stat,del_mu_stat_inf,del_mu_stat_sup,tes, jes, soft_met, bkg_scale ,ttbar_scale,  diboson_scale =BNLL_all_syst_mu(
                                    theshold=threshold,nb_bins=nb_bins,mu_init=mu_init,
                                    weight_exp_hold=weight_exp_hold,label_exp_hold=label_exp_hold,score_exp_hold=score_exp_hold,
                                    weight_test=weight_test,score_test=score_test)
    ...
    if method =="UNLL" or method=="Direct" :
        ...
        negloglike_values = np.array(
            [Cost_nll_syst(mu=mub ,tes=tes) for mub in mu_axis_values] #tes=tes, jes=jes, soft_met=soft_met
        )
        negloglike_mu_hat = Cost_nll_syst(mu=mu ,tes=tes )# tes=tes,  jes=jes, soft_met=soft_met)


    elif method == "BNLL_syst_normal_bkg":
           
        def BNLL_syst_norm_mu_parameter(threshold,nb_bins,weight_exp_hold,label_exp_hold,score_exp_hold,weight_test,score_test):
            weight_ROIscore_exp_holdout = weight_exp_hold[score_exp_hold > threshold]
            label_Roiscore_exp_holdout = label_exp_hold[score_exp_hold > threshold]
            score_ROIscore_exp_holdout = score_exp_hold[score_exp_hold > threshold]
            score_ROIscore_test = score_test[score_test > threshold]
        ...
        negloglike_values = np.array(
            [Cost_nll_syst_norm(mu=mub ,bkg_scale=bkg_scale, ttbar_scale=ttbar_scale, diboson_scale=diboson_scale) for mub in mu_axis_values] #tes=tes, jes=jes, soft_met=soft_met
            )
        negloglike_mu_hat = Cost_nll_syst_norm(mu=mu ,bkg_scale=bkg_scale, ttbar_scale=ttbar_scale, diboson_scale=diboson_scale)# tes=tes,  jes=jes, soft_met=soft_met)
