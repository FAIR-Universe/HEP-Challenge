...
######################################################################################
        ...
        for i in range ( len(bin_list_tamp) ):
            ...
            regression_tes ( self.holdout_set, self.model ,nb_bins=bin_list_tamp[i])
            regression_jes( self.holdout_set, self.model ,nb_bins=bin_list_tamp[i])
            regression_soft_met ( self.holdout_set, self.model,nb_bins=bin_list_tamp[i] )
        """
        #######
        ####for BNLL_all_syst
        #######
        ...
        if Add_info_plot == "True":
            ...
            plt.savefig(
                "current_dir/images/AMS-AUC-mu_VS_Threshold_%s_Train=%s_Holdout=%s_Valid=%s.png"
                % (ModelType, NbTrain, NbHoldout, NbValidation)
            )
            plt.show()
            """
