...
def features_systematics_dependence(dfall, systematics, columns, nb_bins=20, var_lenght=100, n_jobs=5):
    ...
    middle=False
    def process_single_systematic_variation(i, syst_value, syst_idx, column, bin_edges, 
                                          signal_hist_ref, bkg_hist_ref, dfall_copy):
        """
        ...
        try:
            ...
            signal_hist = np.histogram(signal_field, bins=updated_bin_edges, weights=signal_weights, density=True)[0]
            bkg_hist = np.histogram(background_field, bins=updated_bin_edges, weights=background_weights, density=True)[0]
    ...
    for h, column in enumerate(columns):
        ...
        for j, syst_type in enumerate(systematics_list_TJS_type):
            print(f"Processing systematic: {systematics_TJS_list_name[j]} (parallelized)")
            
            # Prepare arguments for parallel processing
            parallel_args = []
            for i in range(var_lenght):
                dfall_copy = dfall.copy()  # Each worker gets its own copy
                parallel_args.append((i, syst_type[i], j, column, bin_edges, 
                                    signal_hist_ref, bkg_hist_ref, dfall_copy))
            
            # Process systematic variations in parallel
            results = Parallel(n_jobs=n_jobs, verbose=1)(
                delayed(process_single_systematic_variation)(*args) for args in parallel_args
            )
        ...
        for j, syst_type in enumerate(systematics_list_TJS_type):
            ...
            for k, (hist_data, label) in enumerate([(signal_hist_diff, "signal"), (bkg_hist_diff, "bkg")]):
        ...
        plt.tight_layout()
        plt.savefig(f"{current_dir}/Images/FeaturesAnalysis_VS_syst/{column}_combined_heatmaps_size{THV_size[0]+THV_size[1]+THV_size[2]}.png", 
                   dpi=150, bbox_inches='tight')
        plt.close(fig)
...
def features_systematics_dependence_new_old(dfall, systematics, columns, nb_bins=20, var_lenght=100):
    ...
    for h, column in enumerate(columns):
        ...
        for j, syst_type in enumerate(systematics_list_TJS_type):
            ...
            for i in range(var_lenght):
                ...
                signal_hist = np.histogram(signal_field, bins=bin_edges, weights=signal_weights, density=True)[0]
                bkg_hist = np.histogram(background_field, bins=bin_edges, weights=background_weights, density=True)[0]
...
def features_systematics_dependence_old (dfall,systematics,columns,nb_bins=20,var_lenght=100):
