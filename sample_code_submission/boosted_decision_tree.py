...
class BoostedDecisionTree:
        ...
        if Load_Classifier == True:
            ...
            if not os.path.exists("%s/Saved_Classifier/BDT" % (current_dir)):
                ...
                self.model.load_model(
                    "%s/Saved_Classifier/BDT/model_BDT_trained_wt_%s_events_seed%s.json"
                    % (current_dir, train_size, seed)
                )
                self.scaler = joblib.load(
                    "%s/Saved_Classifier/BDT/scaler_BDT_trained_wt_%s_events_seed%s.pkl"
                    % (current_dir, train_size, seed)
                )
    ...
    def fit(self, train_data, labels, train_size, seed, weights_train=None):
        ...
        plt.tight_layout()
        plt.savefig(
            "%s/Images/BDT/Features_Importances/BDT_trained_wt_%s_events_seed%s.png"
            % (current_dir, train_size, seed)
        )
        plt.close()
        ...
        plt.savefig(
            "%s/Images/BDT/Features_Permutation/BDT_trained_wt_%s_events_seed%s.png"
            % (current_dir, train_size, seed)
        )
        plt.close()
