...
def compute_mu(
    ...
    ###############################################################
        ...
        def BNLL_syst_mu_parameter_definition (theshold,nb_bins,weight_exp_hold,label_exp_hold,score_exp_hold,score_test,weight_test):
            import os
        ...
        n_obs_test_biaised, sig_exp_hold_unbiaised, bkg_exp_hold_unbiaised=BNLL_syst_mu_parameter_definition(
    ...
    ###############################################################
        ...
        def Cost_nll_syst_norm(mu ,bkg_scale, ttbar_scale, diboson_scale ): # bkg_scale, ttbar_scale, diboson_scale tes, jes, soft_met):
            ...
            def BNLL_syst_norm_mu_computation (mu_init ,bkg_scale_init, ttbar_scale_init, diboson_scale_init): # bkg_scale_init, ttbar_scale_init, diboson_scale_init 
                ...
                m_bnll_syst_norm.limits["mu"] = (0, None)
    ...
    ###############################################################
        ...
        def BNLL_all_syst_mu_parameter_definition (theshold,nb_bins,weight_exp_hold,label_exp_hold,score_exp_hold,score_test,weight_test):
            import os
            from systematic_analysis import Polynomial_Reg_Model_forced_jes_tes,Polynomial_Reg_Model_forced_soft_met
        ...
        n_obs_test_biaised, sig_exp_hold_unbiaised, bkg_scale_ztautau_exp_hold,bkg_ttbar_exp_hold,bkg_diboson_exp_hold=BNLL_all_syst_mu_parameter_definition(
                        threshold=threshold,nb_bins=nb_bins,
                        weight_exp_hold=weight_exp_hold,label_exp_hold=label_exp_hold,
                        score_exp_hold=score_exp_hold,
                        weight_test=weight_test,score_test=score_test)
        ...
        def BNLL_all_syst_mu_computation (mu_init ,tes_init ,jes_init ,soft_met_init, bkg_scale_init, ttbar_scale_init, diboson_scale_init): # bkg_scale_init, ttbar_scale_init, diboson_scale_init ,tes_init ,jes_init ,soft_met_init):
            ...
            m_bnll_all_syst.limits["mu"] = (0, None)
            ...
            m_bnll_all_syst.fixed["soft_met"]=True
    ...
    if method =="UNLL" :
        ...
        negloglike_values = np.array(
            [Cost_nll_syst(mu=mub ,tes=tes, jes=jes,soft_met=soft_met) for mub in mu_axis_values] #tes=tes, jes=jes, soft_met=soft_met
        )
        negloglike_mu_hat = Cost_nll_syst(mu=mu ,tes=te, jes=jes,soft_met=soft_met )# tes=tes,  jes=jes, soft_met=soft_met)
