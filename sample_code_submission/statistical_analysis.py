...
def compute_mu(
    ...
    if (
        saved_info_hold["threshold"] != threshold
        or saved_info_hold["nb_bins"] != nb_bins
    ):
    ...
    ###############################################################
        ...
        mu, del_mu_stat, del_mu_stat_inf, del_mu_stat_sup = UNLL_mu_computation(
            mu_init=mu_init
        )
        tes = tes_init_ref
        jes = jes_init_ref
    ...
    ###############################################################
        ...
        def Cost_bnll(mu):
            N_exp_bnll = Model(
                mu=mu,
                sig=saved_info_hold["signal_hist"],
                bkg=saved_info_hold["bkg_hist"],
            )
        ...
        mu, del_mu_stat, del_mu_stat_inf, del_mu_stat_sup = BNLL_mu_computation(
            mu_init=mu_init
        )
        tes = tes_init_ref
        jes = jes_init_ref
    ...
    ###############################################################
        ...
        def Cost_bnll_syst(mu, tes, jes, soft_met):  # tes, jes, soft_met):
            ...
            bkg_exp_hold_biaised = (
                saved_info_hold[
                    "bkg_hist"
                ]  # saved_info_hold["bkg_hist"] = bkg_exp_hold_unbiaised
                ###Below tes
                + np.array(
                    [
                        Polynomial_Reg_Model_forced_jes_tes(
        ...
        def BNLL_syst_mu_computation(
            mu_init, tes_init, jes_init, soft_met_init
        ...
        mu, del_mu_stat, del_mu_stat_inf, del_mu_stat_sup, tes, jes, soft_met = (
            BNLL_syst_mu_computation(
                mu_init=mu_init,
                tes_init=tes_init_ref,
                jes_init=jes_init_ref,
                soft_met_init=soft_met_init_ref,
            )
        )  # tes_init=1 ,jes_init=1 ,soft_met_init=soft_met_init_ref)
        bkg_scale = bkg_scale_init_ref
        ttbar_scale = ttbar_scale_init_ref
        diboson_scale = diboson_scale_init_ref
    ...
    ###############################################################
        ...
        ):  # bkg_scale_init, ttbar_scale_init, diboson_scale_init

            m_bnll_syst_norm = Minuit(
                Cost_nll_syst_norm,
                mu=mu_init,
                bkg_scale=bkg_scale_init,
                ttbar_scale=ttbar_scale_init,
                diboson_scale=diboson_scale_init,
            )  # bkg_scale=bkg_scale_init, ttbar_scale=ttbar_scale_init, diboson_scale=diboson_scale_init
    ...
    if method == "UNLL":
        ...
        negloglike_mu_hat = Cost_bnll_syst(
            mu=mu, tes=tes, jes=jes, soft_met=soft_met
        )  # tes=tes,  jes=jes, soft_met=soft_met)
...
def calculate_saved_info(
    ...
    ztautau_hist = np.histogram(
        ...
        bins=Bins_edges,
        weights=weight_ROIscore_exp_holdout[
            detailed_labels_Roiscore_exp_holdout == "ztautau"
        ],
    )[0]
    ...
    ttbar_hist = np.histogram(
        ...
        bins=Bins_edges,
        weights=weight_ROIscore_exp_holdout[
            detailed_labels_Roiscore_exp_holdout == "ttbar"
        ],
    )[0]
    ...
    diboson_hist = np.histogram(
        ...
        bins=Bins_edges,
        weights=weight_ROIscore_exp_holdout[
            detailed_labels_Roiscore_exp_holdout == "diboson"
        ],
    )[0]
    ...
    dump(
        ...
        % (current_dir, ModelType, Nb_bins_distrib, THV_size[1], random_seed),
    )
